...
interface ChatCommandOutbox {
   ...
   * row id, so the original idempotency key still dedupes on the gateway. Gated rows are
   * re-stamped with the caller's current connection epoch. Queued successors in the same
   * session shift behind the retried row in their original order, so retrying an ambiguous
   * head can never make younger turns of the conversation overtake it.
   */
...
internal interface ChatOutboxDao {
  ...
  @Query(
    "UPDATE outbox_commands SET status = :status, retryCount = 0, lastError = NULL, createdAtMs = :createdAtMs, " +
      "gatedEpoch = :gatedEpoch WHERE id = :id",
  )
  suspend fun requeueForRetry(
    id: String,
    createdAtMs: Long,
...
class RoomChatCommandOutbox internal constructor(
  ...
  override suspend fun enqueue(
    ...
    // rows commit atomically, so durable admission is all-or-nothing across a crash.
  ...
  override suspend fun requeueForRetry(
    ...
    database.withTransaction {
      val rows = dao.commands(gateway)
      val target = rows.firstOrNull { it.id == id } ?: return@withTransaction

...
class ChatControllerOutboxTest {
      ...
      return when (method) {
        "chat.send" -> {
          ...
          sentAttachmentFileNames +=
            (params["attachments"] as? kotlinx.serialization.json.JsonArray)
        ...
        "chat.history" -> {
          ...
          val explicit =
            (json.parseToJsonElement(historyMessagesJson) as kotlinx.serialization.json.JsonArray).map { it.toString() }
  ...
  fun startupRecoveryFinishesBeforeAHealthFlushCanClaimQueuedRows() =
    runTest {
      ...
      // The recovered ambiguous row holds its session: younger queued work in the same
      // conversation must not overtake an unresolved turn, so nothing is sent yet.
      assertTrue(gateway.sentMessages.isEmpty())
      ...
      // Resolving the ambiguous head (user delete) releases the session's queued successor.
      chat.deleteOutboxCommand("interrupted")
      advanceUntilIdle()
  ...
  fun startupRecoveryFailureBlocksFlushUntilRecoveryCanBeRetried() =
    runTest {
      ...
      assertEquals(ChatOutboxStatus.Failed, outbox.rows.getValue("interrupted").status)
      assertEquals(OUTBOX_DELIVERY_UNCONFIRMED_ERROR, outbox.rows.getValue("interrupted").lastError)
      // The recovered ambiguous row still holds its session; the younger queued row waits for
      // the user to resolve it instead of overtaking a possibly delivered turn.
      assertTrue(gateway.sentMessages.isEmpty())
      assertEquals(ChatOutboxStatus.Queued, outbox.rows.getValue("queued").status)
    }
  ...
  @Test
  fun ambiguousRowBlocksItsSessionWhileOtherSessionsKeepFlushing() =
    runTest {
      val gateway = FakeGateway()
      val outbox = FakeCommandOutbox()
      val now = System.currentTimeMillis()
      ...
      // FIFO holds within the ambiguous session only; unrelated sessions are not starved.
      assertEquals(listOf("independent session"), gateway.sentMessages)
      assertEquals(ChatOutboxStatus.Queued, outbox.rows.getValue("queued-a").status)
  ...
  @Test
  fun retryingAnUnconfirmedHeadKeepsItsSessionOrderAheadOfQueuedSuccessors() =
    runTest {
      val gateway = FakeGateway()
      val outbox = FakeCommandOutbox()
      val now = System.currentTimeMillis()
      ...
      chat.handleGatewayEvent("health", null)
      advanceUntilIdle()
      // The ambiguous head holds the session; nothing is sent yet.
      assertTrue(gateway.sentMessages.isEmpty())

      // Explicit retry must deliver the head before the younger queued turn.

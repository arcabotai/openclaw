...
class RoomChatCommandOutboxTest {
  ...
  fun requeueForRetryRefreshesCreatedAtSoExpirySweepCannotRefailIt() =
    runTest {
      ...
      store.requeueForRetry(gatewayId = "gateway-a", id = stale.id, nowMs = now, gatedEpoch = null)
      store.expireStale("gateway-a", nowMs = now)
  ...
  fun gatedEpochSurvivesPersistenceAndRetryRestamping() =
    runTest {
      ...
      store.requeueForRetry(gatewayId = "gateway-a", id = queued.item.id, nowMs = 20, gatedEpoch = 9L)
  ...
  fun requeueForRetryKeepsSameSessionQueuedSuccessorsBehindTheRetriedRow() =
    runTest {
      ...
      store.requeueForRetry(gatewayId = "gateway-a", id = head.id, nowMs = 1_000_000_000L, gatedEpoch = null)
